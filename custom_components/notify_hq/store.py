...
async def async_get_registry(hass: HomeAssistant) -> NotifyHqStorage:
    """Return battery notes storage instance."""
    task = hass.data.get(DATA_REGISTRY)

    if task is None:

        async def _load_reg() -> NotifyHqStorage:
            registry = NotifyHqStorage(hass)
            await registry.async_load()
            return registry

        task = hass.data[DATA_REGISTRY] = hass.async_create_task(_load_reg())

    return cast(NotifyHqStorage, await task)
